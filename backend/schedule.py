...
#     # Fetch staff details including department and position
#     staff_data = supabase.table('employee') \
#         .select('staff_fname, staff_lname, staff_id, dept, position') \
...
#     # Fetch staff data for the given staff_id to get the reporting manager
#     staff_data = supabase.table('employee') \
...
#     # Get the reporting manager ID for the staff member
#     reporting_manager_id = staff_data.data[0]['reporting_manager']
...
#     # Fetch the reporting manager's data
...
#         return jsonify({"error": "No reporting manager found."}), 404
...
#         # Append the staff data for this member, including dept, position, and reporting manager
#         all_staff_data.append({
#             'staff_name': f"{member['staff_fname']} {member['staff_lname']}",
#             'staff_id': member['staff_id'],
#             'dept': member['dept'],  # Adding department
#             'position': member['position'],  # Adding position
#             'manager': is_manager,
#             'reporting_manager': f"{manager_data.data[0]['staff_fname']} {manager_data.data[0]['staff_lname']}"
#         })

#     # Flatten the schedules to ensure each staff has one long list
#     flattened_schedules = [schedule for sublist in all_schedules for schedule in sublist]

#     # Return the final result as JSON, including reporting manager info
#     return jsonify({
#         "schedules": flattened_schedules,
#         "staff_data": all_staff_data,
#         "reporting_manager": {
#             "name": f"{manager_data.data[0]['staff_fname']} {manager_data.data[0]['staff_lname']}",
#             "staff_id": manager_data.data[0]['staff_id'],
#             "dept": manager_data.data[0]['dept'],
#             "position": manager_data.data[0]['position']
#         }
#     })
